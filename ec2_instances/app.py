...
def lambda_handler(event, context):
    ...
    data = getAllEC2(bearer_token, region);
    count = getCount(data);
    if (count > 15):
        sendEmail(arn);
...
def getAllEC2(bearer_token, region='us-east-1'):
    Headers={'Authorization': bearer_token};
    r = requests.get(f'https://api.cursocloudaws.net/oteador/services/AllInstancesEC2/region/{region}', headers=Headers);
    data = r.json();
    return data;
